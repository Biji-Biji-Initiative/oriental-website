...
| Infisical/Coolify deployment env | `/deploy/oriental-website`; `COOLIFY_ORIENTAL_APPLICATION_UUID=mtrl2z6a7zvoyevxvufpntij`; the potentially known interactive password is represented only as `ADMIN_REVIEW_PASSWORD_HMAC` keyed by the strong `ADMIN_REVIEW_TOKEN` and yields a 30-minute viewer session |
