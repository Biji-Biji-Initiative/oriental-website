...
/**
 * The shared production Convex query predates voice-variant attribution, but
 * its existing per-session query returns the complete stored record. Enrich
 * only missing bulk rows through that read-only path so staging can prove its
 * exact picker cell without deploying shared Convex functions first.
 */
