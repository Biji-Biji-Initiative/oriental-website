...
export function SiteNav() {
  ...
  const navLinkTone = scrolled ? "text-white/62 hover:text-white" : "text-mk-off-black/58 hover:text-mk-anchor-blue";
  const navLinkActiveTone = scrolled
    ? "site-nav__link--active text-white"
    : "site-nav__link--active text-mk-anchor-blue";

  return (
    <>
      <header
        className={cn(
          "fixed top-0 right-0 left-0 z-40 flex items-center justify-between gap-4 px-5 py-4 text-mk-off-black transition duration-300 md:px-9",
      ...
      >
        <a className="flex items-center gap-3 text-sm font-semibold tracking-[0.18em]" href={topHref}>
          <Image
            ...
            width={112}
          />
          <span className={cn(scrolled ? "text-white/48" : "text-mk-off-black/34")}>×</span>
          <span>ORIENTAL</span>
        </a>
        <nav
        ...
        >
          ...
          <a className={cn("site-nav__link relative transition", navLinkTone)} href="/faq">
            FAQ
          </a>
        </nav>
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className={cn(
              "flex items-center gap-2 rounded-full px-4 py-2 text-xs font-semibold tracking-[0.12em] uppercase backdrop-blur transition",
              scrolled
