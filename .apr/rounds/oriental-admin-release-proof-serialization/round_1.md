...
* The live release proof verifies the canonical target is Redis-backed and that spoofed earlier `X-Forwarded-For` hops share exactly one limiter bucket. It deliberately stops before the destructive boundary so it cannot lock out subsequent release work or a real visitor behind the same egress address.
